...
export default function PracticeDetailClient({ practice: initialPractice }: PracticeDetailClientProps) {
  ...
  const handleCommentAdded = async () => {
    // コメントが追加されたら最新データを取得
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/practices/${practice.id}`);
      if (res.ok) {
        const data = await res.json();
        setPractice(data.data);
      }
    } catch (error) {
      console.error('Error refreshing practice:', error);
    }
  ...
  const averageRating = practice.averageRating || 0;
        ...
        {averageRating > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-gray-600">
              {averageRating.toFixed(1)} ({practice.ratingsCount || practice.ratings.length}件の評価)
            </span>
          </div>
        )}
      </div>
